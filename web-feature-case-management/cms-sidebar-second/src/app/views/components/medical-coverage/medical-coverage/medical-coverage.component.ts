...
import { AssignMedicalCoverageComponent, SelectedItem } from './../../../../views/components/medical-coverage/assign-medical-coverage/assign-medical-coverage.component';
...
import { SelectedPlan, MedicalCoverageSelected, PolicyHolderInfo, CoverageSelected } from '../../../../objects/MedicalCoverage';
...
import { MEDICAL_COVERAGES, DISPLAY_DATE_FORMAT } from './../../../../constants/app.constants';
...
export class MedicalCoverageComponent implements OnInit {
    ...
    for (const key of Object.keys(array)) {
      if (array[key].length > 0) {
        array[key].forEach(element => {
          ...
          const medicalCoverageSelected = this.store.getPlansByCoverageId(element.policyHolder.medicalCoverageId).length === 0 ? new MedicalCoverageSelected() : <MedicalCoverageSelected>this.store.getPlansByCoverageId(element.policyHolder.medicalCoverageId);
          const plan = new SelectedPlan(false,
          ...
          const planFG = this.fb.group(plan); const today = moment().format(DISPLAY_DATE_FORMAT);
          const endDate = planFG.get('endDate');

          planFG.get('endDate').setValidators([notBeforeToday(today, endDate),
  ...
  toDisplay(coverage) {
    ...
    const policyAttached = this.selectedCoverages.value.find(selectedCoverage => {
  ...
  onbtnDeleteClicked(index, $event) {
    ...
    console.log("delete: ", this.patientCoverages);
  ...
  addNewItem() {
    ...
    formGroup.get('endDate').setValidators([notBeforeToday(today, endDate),
  ...
  checkCompatibility(current, sibling) {
    ...
    if (
      ...
      (current === insurance && sibling === corporate) ||
      (sibling === insurance && current === corporate)
    ) {
      return false;
    } else if (
      current === corporate &&
  ...
  getSelectedIndex(item) {
    ...
    this.selectedCoverages.value.forEach((coverage, counter) => {
      if (item.get('medicalCoverageId').value === coverage.medicalCoverageId
