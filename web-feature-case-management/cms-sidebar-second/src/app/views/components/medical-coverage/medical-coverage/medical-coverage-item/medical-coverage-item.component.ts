...
export class MedicalCoverageItemComponent implements OnInit {
  ...
  constructor(private apiPatientInfoService: ApiPatientInfoService,
  ...
  initialiseValues() {
    ...
    const isPolicyExpired = !moment(today, DISPLAY_DATE_FORMAT).isSameOrBefore(moment(policyEndDate, DISPLAY_DATE_FORMAT));
    ...
    if (!this.patientCoverageItem.get('isNew').value) {
      ...
      const isHolderExpired = !moment(today, DISPLAY_DATE_FORMAT).isSameOrBefore(moment(holderEndDate, DISPLAY_DATE_FORMAT));
  ...
  subscribeMedicalCoverageValueChanges() {
    ...
    this.patientCoverageItem.get('endDate').setValidators([notBeforeToday(today, endDate),
    ...
    this.patientCoverageItem.valueChanges
      ...
      .subscribe(values => {
        ...
        const validHolderEndDate = moment(endDate, DISPLAY_DATE_FORMAT).isSameOrBefore(moment(policyEndDate, DISPLAY_DATE_FORMAT));

        this.patientCoverageItem.get('startDate').patchValue(startDate);
        this.patientCoverageItem.get('endDate').patchValue(endDate);
        this.patientCoverageItem.get('planSelected').value.registrationRemarks = values.planSelected.registrationRemarks;
          ...
          data => {
            ...
            if (data) {
              const filteredMedicalCoverages = data.payload.filter(element => {
                ...
                return element['status'] === 'ACTIVE' && element['coveragePlans'].length > 0 && moment(today, DISPLAY_DATE_FORMAT).isSameOrBefore(moment(element['endDate'], DISPLAY_DATE_FORMAT));
              });
  ...
  isPlanSelected() {
    this.attachedMedicalCoverages.value.forEach(coverage => {
      if (this.patientCoverageItem.get('medicalCoverageId').value === coverage.medicalCoverageId
        && this.patientCoverageItem.get('planId').value === coverage.planId) {
    ...
    if (this.attachedMedicalCoverages) {
      if (this.patientCoverageItem.get('isNew').value) {
        this.attachedMedicalCoverages.value.map((coverage, index) => {
          if (coverage.medicalCoverageId === this.patientCoverageItem.get('medicalCoverageId').value
            && coverage.planId === this.patientCoverageItem.get('planId').value) { this.attachedMedicalCoverages.removeAt(index); }
        });
      } else {
        this.attachedMedicalCoverages.value.map((coverage, index) => {
          const coverageId = this.policyHolderInfo ? this.policyHolderInfo.id : '-';
          if (coverage.coverageId === coverageId &&
            ...
            coverage.planId === this.policyHolderInfo.planId) { this.attachedMedicalCoverages.removeAt(index); }
        });
      }
    }
  }
  ...
  disableCheckBox() {
    ...
    if (toDisable) {
      ...
      if (!isNew) {
        ...
        const validHolderEndDate = moment(holderEndDate, DISPLAY_DATE_FORMAT).isSameOrBefore(moment(policyEndDate, DISPLAY_DATE_FORMAT)) &&
...
export function notBeforeToday(startDate, endDate: AbstractControl): ValidatorFn {
  return (control: AbstractControl) => {
    ...
    if (isPolicyExpired) {
      return { policyExpired: { value: moment(endDate.value, DISPLAY_DATE_FORMAT), message: 'End date must not be before today.' } };
    } else {
      return null;
    }
  };
...
export function validateAgainstMC(startDate: AbstractControl, endDate: AbstractControl): ValidatorFn {
  return (control: AbstractControl) => {
    ...
    if (isPolicyExpired) {
      ...
      return { mcEndDateAfterPolicy: { value: moment(endDate.value, DISPLAY_DATE_FORMAT), message: 'End date must not be beyond medical coverage end date.' } };
