  ...
  subscribeArrayItemChange() {
    this.paymentFormGroup
      ...
      .subscribe(data => {
        ...
        this.paymentFormGroup.get('totalOutstanding').patchValue(Math.max(0, totalOutstanding).toFixed(2));
        this.paymentFormGroup.get('cashOutstanding').patchValue(Math.max(0, cashOutstanding).toFixed(2));
        this.paymentFormGroup.get('outstanding').patchValue(outstanding.toFixed(2));
      });
  }
