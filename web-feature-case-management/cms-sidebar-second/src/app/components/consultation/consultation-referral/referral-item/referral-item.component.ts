...
export class ReferralItemComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // this.practices = this.store.specialitiesList;


    this.referralItem.get('appointmentDateTime').patchValue(moment(new Date()).format(DB_FULL_DATE_FORMAT_NO_SECOND));
  ...
  subscribeChanges() {
    this.referralItem.valueChanges.pipe(debounceTime(200)).subscribe(value => {
      console.log('changes in referral: ', value);
  ...
  populateFormValues() {
    ...
    if (externalReferral) {
      ...
      // console.log('REFERRAL DETAILS: ', externalReferralDetails);
    ...
    if (this.referralItem.get('doctorId').value && this.referralItem.get('clinicId').value) {
      ...
      const doctorSelected = clinicSelected.doctors.find(function (x) {
        return x.id === doctor;
      });
  ...
  onPracticeSelected(option) {
    ...
    if (option) {
      ...
      // const { data } = this.practices.find(practise => practise.value === option);
      // this.populateClinic(data.clinics);
      ...
      this.clinics = [];
      this.doctors = [];
      // this.setMemoAsNotMandatory();
      // this.setNotMandatoryFields();
    }
  }
  ...
  setMandatoryFields() {
    ...
    const externalReferralDoctor = this.referralItem.get('externalReferralDetails').get('doctorName');
    const externalReferralAddress = this.referralItem.get('externalReferralDetails').get('address');
    const externalReferralDetailsFormGroup = this.referralItem.get('externalReferralDetails');
    ...
    if (
      ...
      practice.setValidators([Validators.required]);
      practice.markAsTouched();
      practice.updateValueAndValidity();
    } else {
    ...
    if (!externalReferral) {
      console.log('internal referral');
      //  DISABLE EXTERNAL REFERRAL COMPONENTS
      externalReferralDoctor.disable();
      externalReferralAddress.disable();
      ...
      if (clinicId.value || doctorId.value || practice.value || memo.value) {
        ...
        clinicId.updateValueAndValidity();
      ...
      externalReferralDoctor.enable();
      externalReferralAddress.enable();
