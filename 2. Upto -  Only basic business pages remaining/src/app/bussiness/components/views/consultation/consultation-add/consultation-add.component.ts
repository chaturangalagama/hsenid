...
// import { VitalTrendComponent } from '../components/vital/vital-trend/vital-trend.component';
// import { ConsultationPatientInfoComponent } from '../components/consultation-patient-info/consultation-patient-info.component';
import { DISPLAY_DATE_FORMAT } from '../../../../../core/constants/app.constants';
...
// import { MedicalCertificateItemsArrayComponent } from '../components/consultation-medical-certificate/medical-certificate-items-array.component';
...
export class ConsultationAddComponent implements OnInit, OnDestroy {
  ...
  private saveConsultationNote() {
    ...
    // if (consultationNote) {
    this.tempStore
      .tempStore(this.store.getPatientVisitRegistryId(), JSON.stringify(consultationNote))
      .subscribe(
        res => {
          // if (res.payload && res.payload['value'])
  ...
  createForm() {
    ...
    this.consultationForm = this.fb.group({
      ...
      drugDispatch: this.fb.group({
        // dispatchDrugDetail: this.consultationFormService.initDrugDispatchDetails()
        dispatchDrugDetail: this.consultationFormService.addMultipleDrugDispatch(5)
      }),
      immunisationGiven: this.fb.group({
        // immunisation: this.consultationFormService.initImmunisation()
        immunisation: this.consultationFormService.addMultipleImmunisation(3)
      }),
      medicalServiceGiven: this.fb.group({
        // medicalServices: this.consultationFormService.initMedicalServiceGiven()
        medicalServices: this.consultationFormService.addMultipleMedicalService(5)
      }),
      issuedMedicalTest: this.fb.group({
        // issuedMedicalTestDetails: this.consultationFormService.initIssuedMedicalTestDetails()
        issuedMedicalTestDetails: this.consultationFormService.addMultipleIssuedMedicalTestDetails(3)
      }),
