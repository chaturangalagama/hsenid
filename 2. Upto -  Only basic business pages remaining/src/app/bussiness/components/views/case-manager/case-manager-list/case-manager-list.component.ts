...
export class CaseManagerListComponent implements OnInit {
  ...
  setPage(pageInfo) {
    ...
    // this.populateData(
    //   [{"id":"C1000","patientId":"1234","patientNRIC":null,"patientName":"chatura","clinicId":"1234","visitIds":["1111","222"],"salesOrderEntity":null,"purchasedPackage":null,"dispatchEntity":null,"status":"OPEN","createdDate":null}]
    // );
    ...
    this.apiCaseManagerService.getCaseList(this.store.clinicCode, this.searchBody, this.page).subscribe(pagedData => {
  ...
  populateData(data) {
    this.caseResponse = data.map((payload, index) => {
      const tempCase = {
        date:moment(moment(payload.createdDate, 'DD-MM-YYYYT00:00:00')).format('DD-MM-YYYY'),
