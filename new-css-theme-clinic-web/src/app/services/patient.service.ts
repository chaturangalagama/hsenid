...
import { ID_TYPES } from '../constants/id.types';
...
export class PatientService {
  ...
  createPatientDetailHistoryFormGroup(): FormGroup{
    return this.fb.group({
      ...
      historyDetailFormGroup: this.fb.group({patientInfo: {},
        consultationInfo: {},
        paymentInfo: {},
        gstValue: 0.07,
  ...
  checkBasicDetailInfo(patientInfo, formGroup: FormGroup){
        ...
        patientInfo.address.address = formGroup.get('line1').value + (formGroup.get('line1').value.endsWith('\n') ? '' : '\n') + formGroup.get('line2').value;
  ...
  checkEmergencyContactInfo(patientInfo, formGroup: FormGroup){
    ...
    patientInfo.emergencyContactNumber.relationship = formGroup.get('relationship').value ?  formGroup.get('relationship').value : null;
  ...
  checkCompanyInfo(patientInfo, formGroup: FormGroup){
    ...
    patientInfo.company.address = formGroup.get('line1').value + (formGroup.get('line1').value.endsWith('\n') ? '' : '\n') + formGroup.get('line2').value;
...
// 4) Validating email address
    ...
    return (control: AbstractControl) => {
      ...
      if (idValue.length > 0) {
        ...
        return apiPatientInfoService
          ...
          }, err => this.handleError), catchError(this.handleError));
      } else {
        return of(null);
      }
    };
  ...
  validateNumber(control: FormGroup) {
    ...
    try {
      ...
      // if (phoneUtil.isValidNumber(result) && NUMBER_PATTERN.test(number)) {
