...
export class ConsultationFormService {
  ...
  resetForm() {
    if (
      ...
      this.itemDispatchDetails.controls = [];
  ...
  createConsultationFormGroup(){
    return this.fb.group({
      ...
      dispatchItemEntities:  this.fb.array([]), // ----> to change to dispatchItemEntities
    })
  }
  ...
  newItemDispatchDetail(data){
    ...
    return this.fb.group({
      ...
      quantity: '',
      planId: '',
      oriTotalPrice: data.sellingPrice.price,
      batchNo: '',
      expiryDate: '',
      adjTotalPrice: '',
      remarks: ''
  ...
  addDrugDispatchDetails(
    ...
    const a = this.drugDispatchDetails;
    
    console.log("DRUG DISPATCH DETAILS build: ",a);
    if (this.drugDispatchDetails) {
      console.log("DRUG DISPATCH DETAILS : ",this.drugDispatchDetails);
  ...
  buildDrugDispatchDetails(option) {

    console.log("BUILD DRUG: ",option);
    ...
    drugId.patchValue(option.id);
    code.patchValue(option.code);

    const a = this.drugDispatchDetails.value;
  ...
  // DIAGNOSIS
  initDiagnosis() {
    const id = new FormControl('00001');
  ...
  checkFollowUp(consultation){
    ...
    if(consultation.consultationFollowup)
    {
        ...
        if (
          currentPatientFollowUp.patientId === '' ||
          !currentPatientFollowUp.patientId ||
