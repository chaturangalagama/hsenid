...
export class PaymentPrescriptionComponent implements OnInit {
  ...
  subscribeBatchNoValueChange(item: FormGroup, batchNo: string) {
    const detail = this.store.getDrugList().find(detail => detail.id === item.value.drugCode);
    if (!detail) {
      return;
    }

    const formArray = item.parent as FormArray;
    const formArrayValue: Array<any> = formArray.value;
    let requestQuantity = item.value.quantity;

    const sameDrugItems = formArrayValue.filter(i => i.drugCode === detail.id && i.batchNo === item.value.batchNo);
    if (sameDrugItems.length > 1) {
      requestQuantity = sameDrugItems.reduce((sum, item) => (sum += item.quantity), 0);
    }
    ...
    this.apiPatientVisitService.checkDrugBatchNo(this.store.getClinicId(), [drugUsage]).subscribe(
      res => {
  ...
  subscribeDrugArrayItemValueChanges(item, values) {
    ...
    this.apiPatientVisitService.paymentCheckDrug(this.store.getPatientVisitRegistryId(), drugPaymentCheck).subscribe(
      res => {
        ...
        if (!quantity) {
          console.log('quantity is not valid amount, not trigger inventory api');
          return;
        }
        ...
        const sameDrugItems = formArrayValue.filter(
          item => item.drugCode === detail.id && item.batchNo === values.batchNo
        );
        if (sameDrugItems.length > 1) {
          requestQuantity = sameDrugItems.reduce((sum, item) => (sum += item.quantity), 0);
        }
  ...
  newDrugArrayItem(): FormGroup {
    ...
    item
      ...
      .subscribe(values => {
        this.subscribeBatchNoValueChange(item, values);
      });
