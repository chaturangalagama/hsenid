...
import { Component, OnInit, HostListener, ViewChild, Testability, Input, EventEmitter, Output } from '@angular/core';
import {
  FormGroup,
  FormBuilder,
  FormArray,
...
import { DISPLAY_DATE_FORMAT, DB_FULL_DATE_FORMAT, DB_FULL_DATE_TIMEZONE, INPUT_DELAY } from '../../../../constants/app.constants';
...
export class PatientDispensingComponent implements OnInit {
              ...
              forkJoin(observableBatch).subscribe(
                coverageArr => {
                  ...
                  attachedMedicalCoverage.forEach(element => {
                    ...
                    if (coverage.type === 'CHAS' || coverage.type === 'MEDISAVE') {
                      /*** Disable Auto Check MHCP Balance ***/
                      // this.apiPatientVisitService.checkMhcpBalance(element.planId, this.patientInfo.userId.number).subscribe(
                      //   response => {
      ...
      arr => {
        ...
        const routerSubscription = this.router.events.subscribe(event => {
          if (event instanceof NavigationStart) {
            ...
            console.log("NAVIGATE: ",this.navigatingUrl);
              ...
              if(this.consultationInfo.medicalReferenceEntity)
              {
                this.paymentRequestInfo = this.consultationInfo.medicalReferenceEntity;
              ...
              this.apiPatientVisitService
                .payment(this.store.getPatientVisitRegistryId(), this.paymentRequestInfo)
                .subscribe(
                  res => {
                    this.isSave = true;
  ...
  onBtnSaveClicked() {
    ...
    // const nextTab = new String('Printing');
